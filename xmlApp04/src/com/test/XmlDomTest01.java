...
public class XmlDomTest01
{
	 public static void main(String[] args)
	{
		 ...
		 try
		{
			...
			//    getLength() 메소드를 통해 확인
			
			// 테스트
			// System.out.println(vehicleNodeList.getLength());
			// 10
			
			for (int i = 0; i < vehicleNodeList.getLength(); i++)	// 10 번 반복
			{
				// item() 메소드는 해당 파라미터에 해당하는 인덱스에 위치한 노드 접근 메소드	
				Node vehicleNode = vehicleNodeList.item(i);	// i 0 ~ 9
				
				// 캐스트 연산자 이용
				// Node 객체를 Element 객체로 변환 가능
				// Node 는상위자료형 Element 는 하위 자료형으로다룸
